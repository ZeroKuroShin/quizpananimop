...
// =========================================================================
// BANK DATA SOAL (9 KATEGORI x 50 SOAL = 450 SOAL)
// =========================================================================
const questionBank = {
    "Matematika": [
        ...
        { q: "Jika sebuah segitiga memiliki alas 10 cm and tinggi 8 cm, berapa luasnya?", a: "40 cm²" },
    ...
    "Pengetahuan IPA": [
        ...
        { q: "Lapisan gas yang menyelihuti planet bumi dinamakan?", a: "Atmosfer" },
    ...
    "Lagu": [
        ...
        { q: "Siapa penyanyi Indonesia penyanyi lagu hit global 'Glimpse of Us'?", a: "Joji" },
        ...
        { q: "Lagu daerah Sumatra Utara yang mengisahkan kerinduan pada kampung halaman bernama?", a: "Sinanggar Tulo" },
        ...
        { q: "Siapa penyanyi lagu legendaris 'My Heart Will Go On'?", a: "Celine Dion" },
        ...
        { q: "Lagu 'Kemesraan' dipopulerkan bersama oleh musisi legendaris Iwan Fals dan...?", a: "Rafika Duri" },
    ...
    "Sejarah Dunia": [
        ...
        { q: "Nama peradaban kuno pembuat undang-undang tertulis tertua 'Eye for an eye' adalah?", a: "Babilonia" },
    ...
    "Fun Fact": [
        ...
        { q: "Negara mana yang memiliki zona waktu terbanyak di dunia?", a: "Prancis" },
        { q: "Organ tubuh manusia mana yang bisa tumbuh kembali secara mandiri meski dipotong sebagian?", a: "Hati / Liver" },
        { q: "Hewan apa yang memegang rekor waktu tidur terlama dalam sehari?", a: "Koala" },
        { q: "Menara Eiffel di Paris bisa bertambah tinggi saat musim panas karena pemuaian besi sebesar?", a: "15 cm" },
        ...
        { q: "Hujan teraneh di dunia, yaitu hujan katak dan ikan kecil, dipicu oleh cuaca?", a: "Puting Beliung / Water spout" },
        { q: "Hewan melata siput diketahui bisa tidur tanpa bangun selama?", a: "3 Tahun" },
        { q: "Otot terkuat di dalam tubuh manusia berdasarkan ukuran beratnya adalah otot?", a: "Rahang" },
        ...
        { q: "Warna kotak hitam (black box) perekam data penerbangan pesawat sebenarnya berwarna?", a: "Oranye Jingga" },
        { q: "Jenis cairan tubuh manusia yang bisa melarutkan silet pisau cukur baja adalah cairan?", a: "Asam Lambung" },
        { q: "Berapa jumlah kelopak mata yang dimiliki sebuah unta gurun?", a: "3 Kelopak" },
        ...
        { q: "Sidik lidah manusia bersifat sama seperti sidik jari, yaitu bersifat?", a: "Unik" },
    ...
    "Geologi": [
        ...
        { q: "Skala yang digunakan untuk mengukur kekuatan gempa bumi adalah?", a: "Skala Richter" },
        ...
        { q: "Mineral alami yang memiliki tingkat kekerasan tertinggi di bumi adalah?", a: "Intan / Berlian" },
        ...
        { q: "Sebutkan nama patahan tektonik aktif terbesar di California AS!", a: "Patahan San Andreas" },
    ...
    "Bisnis": [
        ...
        { q: "Siapa tokoh pendiri e-commerce raksasa Amazon?", a: "Jeff Bezos" },
        ...
        { q: "Aktivitas memata-matai atau menilai kelebihan produk milik kompetitor bisnis saingan disebut?", a: "Riset Pasar" },
        ...
        { q: "Istilah penurunan nilai fisik aset tetap seiring waktu pemakaian dinamakan?", a: "Depresiasi / Penyusutan" },
...
function initNetworkSync() {
    ...
    pubnub.addListener({
        message: function(event) {
            ...
            else if (data.type === "FORCE_REJOIN_REQUEST" && gameState.role === "player") {
                const teamName = gameState.players.length < 4 ? "Tim A" : "Tim B";
                const myProfile = { 
                    name: gameState.currentUser, 
                    isReady: document.getElementById('btn-ready').innerText.includes("SAYA READY"), 
                    score: 0, 
                    team: teamName 
                };
                pubnub.publish({
                    channel: gameState.gameCode,
                    message: { type: "PLAYER_JOINED", player: myProfile }
                });
            }
        }
    });
...
function loginAsAdmin() {
    ...
    if(u === 'admin' && p === 'admin') {
        ...
        gameState.gameCode = generateRandomCode(); 
...
function renderLobby() {
    ...
    if ("admin" === gameState.role) {
        ...
        document.getElementById("btn-refresh-peserta").classList.remove("hidden-screen"); 
    } else {
        ...
        document.getElementById("btn-refresh-peserta").classList.add("hidden-screen"); 
    }
    updateParticipantList();
}

function updateParticipantList() {
    const e = document.getElementById("lobby-participants-list");
    e.innerHTML = "";
    document.getElementById("participant-count").innerText = gameState.players.length;
    
    gameState.players.forEach(t => {
        const n = document.createElement("div");
        n.className = `p-3 rounded-lg border text-center transition ${t.isReady ? "bg-emerald-950/40 border-emerald-500 text-emerald-300" : "bg-slate-900 border-slate-700 text-slate-400"}`;
        n.innerHTML = `<p class="font-bold block truncate">${t.name}</p><span class="text-[10px] uppercase font-semibold">${t.isReady ? "Ready" : "Unready"}</span>`;
        e.appendChild(n);
    });
}

function requestRejoinFromPlayers() {
    gameState.players = [];
    updateParticipantList();
    pubnub.publish({
        channel: gameState.gameCode,
        message: { type: "FORCE_REJOIN_REQUEST" }
    });
}
...
function updateScoreboard() {
    const e = document.getElementById("game-scoreboard-list");
    if(!e) return;
    e.innerHTML = "";
    
    if("solo" === gameState.mode) {
        gameState.players.sort((e, t) => t.score - e.score);
        gameState.players.forEach(t => {
            const n = document.createElement("div");
            n.className = "flex justify-between items-center p-3 bg-slate-900 border border-slate-700 rounded-xl";
            n.innerHTML = `<span class="font-medium text-sm text-slate-200">${t.name} ${t.name===gameState.currentUser?"(Kamu)":""}</span><span class="font-bold text-indigo-400 bg-indigo-950/50 px-3 py-1 rounded-md text-sm border border-indigo-900">${t.score} Pts</span>`;
            e.appendChild(n);
        });
    } else {
        let t = {};
        ...
        Object.keys(t).forEach(n => {
            ...
            s.className = "p-3 bg-slate-900 border border-slate-700 rounded-xl space-y-1";
            let a = gameState.players.filter(e => e.team === n).map(e => e.name).join(", ");
            s.innerHTML = `<div class="flex justify-between items-center"><span class="font-bold text-sm text-emerald-400">${n}</span><span class="font-bold text-emerald-400 bg-emerald-950/50 px-3 py-1 rounded-md text-sm border border-emerald-900">${t[n]} Pts</span></div><p class="text-[11px] text-slate-400 italic truncate">Anggota: ${a}</p__>`;
            e.appendChild(s);
        });
    }
}
...
function switchAuthTab(t) {
    "admin" === t ? (
        document.getElementById("tab-admin").className = "w-1/2 py-2 text-center rounded-md bg-indigo-600 font-medium transition text-white",
        document.getElementById("tab-player").className = "w-1/2 py-2 text-center rounded-md text-slate-400 font-medium transition",
        document.getElementById("form-admin").classList.remove("hidden-screen"),
        document.getElementById("form-player").classList.add("hidden-screen")
    ) : (
        document.getElementById("tab-player").className = "w-1/2 py-2 text-center rounded-md bg-indigo-600 font-medium transition text-white",
        document.getElementById("tab-admin").className = "w-1/2 py-2 text-center rounded-md text-slate-400 font-medium transition",
        document.getElementById("form-player").classList.remove("hidden-screen"),
        document.getElementById("form-admin").classList.add("hidden-screen")
    );
}
